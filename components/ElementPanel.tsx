...
const InfoRow: React.FC<{ label: string; value: React.ReactNode; tooltip?: string }> = ({ label, value, tooltip }) => (
  <div className="py-2 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center text-sm">
    ...
    <dd className="text-right text-gray-900 dark:text-white">{value || 'N/A'}</dd>
  </div>
);
...
const ElementPanel: React.FC<ElementPanelProps> = ({ element, isFavorite, onClose, onToggleFavorite }) => {
  ...
  return (
    <div
      className="fixed inset-0 bg-gray-900 bg-opacity-70 dark:bg-black dark:bg-opacity-70 z-50 flex items-center justify-center p-4"
      onClick={onClose}
      role="dialog"
      aria-modal="true"
    ...
    >
      <div
        ref={panelRef}
        tabIndex={-1}
        className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto text-gray-900 dark:text-white relative outline-none"
        onClick={e => e.stopPropagation()}
      >
        <button onClick={onClose} className="absolute top-4 right-4 text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white transition-colors z-10">
