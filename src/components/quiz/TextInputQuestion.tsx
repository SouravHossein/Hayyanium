      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        <div className="relative">
          <input ref={inputRef} type="text" inputMode={question.answerType === 'number' ? 'numeric' : 'text'} value={showFeedback ? (userAnswer || '') : value} onChange={handleChange} disabled={disabled} placeholder={question.answerType === 'number' ? 'Type a number...' : 'Type your answer...'} autoComplete="off"
            className={`w-full min-h-12 rounded-xl border-2 px-4 sm:px-5 py-3.5 text-base sm:text-lg font-semibold text-center outline-none transition-all duration-300 ${showFeedback ? (isCorrect ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300' : 'border-red-500 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-300') : 'border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20'}`} aria-label="Your answer" />
          {showSugg && !disabled && (
            <div className="absolute left-0 right-0 top-full z-20 mt-1 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 shadow-xl overflow-hidden max-h-64 overflow-y-auto">
              {suggestions.map(s => (
                <button key={s} type="button" onClick={() => { setValue(s); setShowSugg(false); onAnswer(s); }} className="w-full min-h-11 px-4 py-2.5 text-left text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-cyan-50 dark:hover:bg-cyan-900/20 transition-colors border-b last:border-b-0 border-gray-100 dark:border-gray-700">{s}</button>
              ))}
            </div>
          )}
        </div>
        {!showFeedback && (
          <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto_auto] items-center gap-2.5">
            <button type="submit" disabled={!value.trim() || disabled} className="min-h-12 w-full rounded-xl bg-gradient-to-r from-cyan-500 to-blue-500 px-6 py-3 text-white font-bold text-sm shadow-lg shadow-cyan-500/25 transition-all hover:shadow-cyan-500/40 hover:scale-[1.02] disabled:opacity-40 disabled:hover:scale-100 disabled:shadow-none">Submit Answer</button>
            {!hintRevealed && <button type="button" onClick={onRevealHint} className="min-h-12 rounded-xl border-2 border-amber-300 dark:border-amber-600 bg-amber-50 dark:bg-amber-900/20 px-4 py-3 text-sm font-semibold text-amber-700 dark:text-amber-300 hover:bg-amber-100 dark:hover:bg-amber-900/40 inline-flex items-center justify-center gap-2"><Lightbulb className="h-4 w-4" /> Hint</button>}
            <button type="button" onClick={onSkip} className="min-h-12 rounded-xl border-2 border-gray-200 dark:border-gray-600 px-4 py-3 text-sm font-semibold text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700/50 inline-flex items-center justify-center gap-2"><ArrowRight className="h-4 w-4" /> Skip</button>
          </div>
        )}
      </form>
