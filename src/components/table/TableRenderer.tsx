  ...
  return (
    ...
    >
      ...
      <div className={`flex flex-col items-center justify-center h-full w-full ${detailLevel === 'small' ? 'opacity-90' : ''}`}>
        ...
        {(detailLevel === 'medium' || detailLevel === 'large') && !compact && (
          <>
            <span className="text-[8px] sm:text-[10px] truncate max-w-full leading-none mt-0.5 font-medium opacity-90">
              {element.name}
            </span>
            <span className="text-[7px] sm:text-[9px] opacity-75 tabular-nums">
              {atomicMass}
            </span>
          </>
        )}
